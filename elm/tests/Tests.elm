...
stepTests =
    let
        sList = SelectList.fromLists ["a", "b", "c"] "d" ["e"]
    in
    ...
    describe "The step function"
        [ test "step forward" <| 
            \_ ->
                let
                    newSList = step Forwards sList
                    expected = fromLists ["a", "b", "c", "d"] "e" []
                in
                    Expect.equal newSList expected
        , test "step forward into invalid state returns same sList" <|
            \_ ->
                let
                    newSList = step Forwards <| step Forwards sList
                    expected = fromLists ["a", "b", "c", "d"] "e" []
                in
                    Expect.equal newSList expected
        , test "step backward" <|
        ...
        , test "step backward into invalid state returns same sList" <|
            \_ ->
                let
                    newSList = step Backwards <| step Backwards <| step Backwards <| step Backwards <| sList
                    expected = fromLists [] "a" ["b", "c", "d", "e"]
                in
                    Expect.equal newSList expected                
        ]
...
selectAtTests =
    let
        ...
        singleton = fromLists [] "a" []
        startSelected = fromLists [] "a" ["b", "c", "d", "e"]
        endSelected = fromLists ["a", "b", "c", "d"] "e" []
    in
...
mkSelectAtTest description element sList expected =
    test description <|
        \_ ->
            let
                newSList = selectAt element sList
            in
                Expect.equal newSList expected
